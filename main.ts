...
import { fromModelToBook, verifyAuthors } from "./utils.ts";
...
const handler = async (request: Request): Promise<Response> => {
  ...
  if (method === "GET") {
      ...
      if (titulo) {
        ...
        const libros = await Promise.all(
          librosModels.map((bookModel) =>
            fromModelToBook(bookModel, AuthorsCollection)
          ),
        );
    ...
    if (path === "/libro") {
      ...
      const book = await fromModelToBook(bookModel, AuthorsCollection);
    ...
    if (path === "/libro") {
      ...
      const book = await fromModelToBook(
        {
          _id: insertedId,
          title,
          authors: authors.map((id: string) => new ObjectId(id)),
